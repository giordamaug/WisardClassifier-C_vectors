...
parser.add_argument('-d', "--debug", metavar='<debug>', type=bool, help='enable progress monitoring (enabled by default)', default=False,required=False)
...
def main(argv):
    ...
    if datafile.endswith('.arff'):
        ...
        X, y = load_svmlight_file(open(datafile, "rb"))
        if sps.issparse(X):
            X = X.toarray()
